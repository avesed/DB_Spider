...
from module1 import User_Agents
from module2 import MongoDBHelper
    ...
    def save_txt(self,movies,id):
        ...
        if len(all_movies)==0:
            for movie in movies:
                id=id+1
                movie['_id']=id
                txt.write((id).encode('UTF-8'))
                txt.write((movies).encode('UTF-8'))
                txt.close()
